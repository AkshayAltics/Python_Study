...
demo_df=pd.read_excel("f:/CodeRepo/titanic3.xls","titanic3",index_col=None,na_values=['na'])
print(demo_df.head())
print(demo_df.describe())
print(demo_df.drop(['ticket','cabin','body','boat'],axis=1).head())
print(demo_df.isnull().sum())
print(pd.value_counts(demo_df["survived"]).plot.bar())
print(demo_df.groupby(["sex"]).mean())
print(demo_df.groupby(["sex","pclass"]).mean())
print(demo_df.groupby(["sex","pclass"]).mean())
...
ls=df.sort_values('record_high', ascending=False)
